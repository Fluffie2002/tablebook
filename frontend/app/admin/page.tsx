...
import { restaurantPerformance } from '@/lib/mock-data';
    ...
    <PageTransition>
      ...
      {overview.isLoading || !overview.data ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <KpiCard title="Occupancy" numericValue={overview.data.occupancyRate ?? 78} suffix="%" change={`${overview.data.conversionRate}% approval rate`} trend="neutral" icon={Percent} />
          <KpiCard title="Active venues" value={String(overview.data.activeRestaurants)} change="5 cities across Malaysia" trend="neutral" icon={Utensils} />
          <KpiCard title="Avg. party size" numericValue={overview.data.avgPartySize ?? 3.4} decimals={1} change="Advance bookings trending up" trend="up" icon={Users} />
        </div>
      )}
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        ...
        {trends.data ? (
          <OccupancyChart data={trends.data.map((d) => ({ date: d.date, occupancy: d.occupancy ?? 70 }))} />
        ) : (
            ...
            {(popularTimes.data ?? []).map((item) => (
              <div key={item.timeSlot}>
                ...
                <div className="h-2 rounded-full bg-[#f0f4f8]">
                  <div className="h-2 rounded-full bg-[#0ea5e9]" style={{ width: `${Math.min(item.reservations, 130) / 1.3}%` }} />
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <RestaurantPerformanceTable data={restaurantPerformance} />
        </div>
